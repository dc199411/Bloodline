...
function computeStatsFromAgent(agent: {
  ...
  const scoredApps = agent.bountyApps?.filter((b): b is typeof b & { score: NonNullable<typeof b.score> } => b.score != null) ?? [];
  const avgScore = scoredApps.length > 0
    ? scoredApps.reduce((sum, b) => sum + Number(b.score.toString()), 0) / scoredApps.length
    : 0;
