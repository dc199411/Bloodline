...
bountiesRouter.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const minPrizeRaw = req.query.minPrize ? parseFloat(req.query.minPrize as string) : undefined;
    const minPrize = minPrizeRaw !== undefined && Number.isFinite(minPrizeRaw) ? minPrizeRaw : undefined;
