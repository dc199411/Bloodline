...
export function useDangerAgents() {
  const fallback = MOCK_AGENTS.filter((a) => a.stage === "danger");
  const result = useFetchWithFallback<{ agents: Agent[] }>(
    "/agents/danger",
    { agents: fallback }
  );
  const agents = Array.isArray(result.data?.agents) ? result.data.agents : fallback;
  return {
    data: agents,
    loading: result.loading,
    error: result.error
...
export function useLeaderboard() {
  ...
  const leaderboard = Array.isArray(result.data?.leaderboard) ? result.data.leaderboard : fallback;
  return {
    data: leaderboard,
    loading: result.loading,
    error: result.error
