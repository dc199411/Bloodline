...
export default function HomePage() {
  ...
  const { data: leaderboard, loading: loadingLeaderboard } = useLeaderboard();
  const { data: posts, loading: loadingPosts } = useSocialFeed();

  const topAgents = leaderboard.slice(0, 5);
  const recentPosts = posts.slice(0, 3);
  const loading = loadingDanger || loadingAgents || loadingLeaderboard || loadingPosts;
  ...
  return (
    <div className="flex flex-col gap-4 px-4 py-4">
      ...
      {loading ? (
        ...
        >
          {topAgents.map((agent: Agent, i: number) => (
            <Link
              key={agent.id}
              href={`/agent/${agent.id}`}
